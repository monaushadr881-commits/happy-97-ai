/**
 * HAPPY X — skills-runtime v3 (Reserved, Phase 3.1).
 * Auth-guarded server fns for the skills runtime. All handlers currently
 * return the shared NOT_IMPLEMENTED sentinel via skillsRuntimeService.
 */
import { createServerFn } from "@tanstack/react-start";
import { requireSupabaseAuth } from "@/integrations/supabase/auth-middleware";
import { makeServiceContext } from "@/services/core/context";
import { toAppError } from "@/services/core/errors";
import { skillsRuntimeService as svcRef } from "@/services/domain/roadmap.service";

type AuthCtx = { supabase: Parameters<typeof makeServiceContext>[0]["supabase"]; userId: string; claims?: Record<string, unknown> };
const svc = (ctx: AuthCtx) => makeServiceContext({ supabase: ctx.supabase, userId: ctx.userId, claims: ctx.claims });
const guard = <T>(fn: () => Promise<T>) => fn().catch((e) => { throw toAppError(e); });

export const apiSkillsStatus = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth])
  .handler(async ({ context }) => guard(() => svcRef.status(svc(context))));
export const apiSkillsList = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth])
...
export const apiSkillsHealth = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth])
  .handler(async ({ context }) => guard(() => svcRef.health(svc(context))));
export const apiSkillsLive = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth])
  .handler(async ({ context }) => guard(() => svcRef.live(svc(context))));
export const apiSkillsHistory = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth])
...
export const apiSkillsExecute = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth])
