/**
 * /builder/media — HAPPY Media Builder™
 *
...
import {
  Newspaper, Rss, FileText, Video, Mic, Radio, MessageSquare, Search,
...
export const Route = createFileRoute("/_authenticated/builder/media")({
  head: () => ({
    meta: [
      { title: "Media Builder — HAPPY X" },
      { name: "robots", content: "noindex" },
    ],
  }),
  component: MediaBuilderRoute,
});

type PresetId =
  | "news" | "blogs" | "videos" | "podcast" | "live" | "comments" | "seo" | "analytics";

const PRESETS: { id: PresetId; label: string; icon: React.ReactNode; hint: string }[] = [
  { id: "news",      label: "News",      icon: <Newspaper className="h-4 w-4" />,     hint: "Desk · categories · breaking · sourcing · byline." },
  { id: "blogs",     label: "Blogs",     icon: <FileText className="h-4 w-4" />,      hint: "Posts · authors · tags · series · scheduling." },
  { id: "videos",    label: "Videos",    icon: <Video className="h-4 w-4" />,         hint: "Uploads · playlists · thumbnails · captions." },
  { id: "podcast",   label: "Podcast",   icon: <Mic className="h-4 w-4" />,           hint: "Episodes · seasons · RSS · show notes · guests." },
  { id: "live",      label: "Live",      icon: <Radio className="h-4 w-4" />,         hint: "Streams · chat · guests · replays · latency." },
  { id: "comments",  label: "Comments",  icon: <MessageSquare className="h-4 w-4" />, hint: "Threads · moderation · reports · reactions." },
  { id: "seo",       label: "SEO",       icon: <Search className="h-4 w-4" />,        hint: "Meta · sitemap · schema · redirects · canonicals." },
  { id: "analytics", label: "Analytics", icon: <BarChart3 className="h-4 w-4" />,     hint: "Reads · watch time · listens · conversion." },
];

const INTRO: Record<PresetId, string> = {
  news:      "Describe the desk · category · breaking · byline rule.",
  blogs:     "Describe the post · author · tag · schedule.",
  videos:    "Describe the video · playlist · caption · thumbnail.",
  podcast:   "Describe the episode · season · RSS · guest.",
  live:      "Describe the stream · chat · guest · replay rule.",
  comments:  "Describe the thread · moderation · report · reaction.",
  seo:       "Describe the meta · schema · sitemap · redirect.",
  analytics: "Describe the report · metric · dimension · window.",
};

interface LogLine { id: string; at: string; kind: "log" | "warn" | "err"; text: string }

function MediaBuilderRoute() {
  const [preset, setPreset] = React.useState<PresetId>("news");
  ...
  const { submit: __submitPrompt } = useBuilderPrompt({ surface: "media", onLog: pushLog });
  ...
  return (
    <Container className="py-6 md:py-10">
      <header className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            <Rss className="h-4 w-4" /> HAPPY Media Builder
          </div>
          <h1 className="text-2xl md:text-3xl font-semibold tracking-tight">
            One canonical HAPPY — news, blogs, video, podcast, live
          </h1>
      ...
      <section aria-label="Media presets" className="flex flex-wrap gap-2">
        {PRESETS.map((p) => (
          <Button
            key={p.id}
            size="sm"
      ...
      <div className="mt-6 grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_340px] gap-6">
        ...
        <main className="space-y-4 min-w-0">
          ...
          <Tabs defaultValue="overview" className="w-full">
            <TabsList>
              ...
              <TabsTrigger value="editorial" className="gap-1"><Newspaper className="h-4 w-4" />Editorial</TabsTrigger>
              <TabsTrigger value="media"     className="gap-1"><Video className="h-4 w-4" />Media</TabsTrigger>
              <TabsTrigger value="live"      className="gap-1"><Radio className="h-4 w-4" />Live</TabsTrigger>
              <TabsTrigger value="community" className="gap-1"><MessageSquare className="h-4 w-4" />Community</TabsTrigger>
            ...
            <TabsContent value="overview"  className="mt-3">
              <div className="rounded-lg border bg-background p-6 grid place-items-center text-sm text-muted-foreground text-center min-h-[240px]">
                <div className="flex flex-col items-center gap-2">
                  {active.icon}
                  <div>{active.label} · HAPPY renders media state from the Business runtime.</div>
                </div>
              </div>
            </TabsContent>
            <TabsContent value="editorial" className="mt-3"><p className="text-sm text-muted-foreground">News · blogs · authors · categories · schedule.</p></TabsContent>
            <TabsContent value="media"     className="mt-3"><p className="text-sm text-muted-foreground">Videos · playlists · podcast episodes · RSS.</p></TabsContent>
            <TabsContent value="live"      className="mt-3"><p className="text-sm text-muted-foreground">Streams · chat · guests · replays · latency.</p></TabsContent>
            <TabsContent value="community" className="mt-3"><p className="text-sm text-muted-foreground">Comments · threads · moderation · reports · reactions.</p></TabsContent>
            <TabsContent value="seo"       className="mt-3"><p className="text-sm text-muted-foreground">Meta · sitemap · schema · canonicals · redirects.</p></TabsContent>
            <TabsContent value="analytics" className="mt-3"><p className="text-sm text-muted-foreground">Reads · watch time · listens · conversion.</p></TabsContent>
        ...
        <aside className="space-y-3">
          <div className="flex items-center gap-2 text-sm font-medium">
            <ScrollText className="h-4 w-4" /> Media Log
          </div>
