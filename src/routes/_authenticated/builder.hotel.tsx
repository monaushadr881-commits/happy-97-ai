/**
 * /builder/hotel — HAPPY Hotel Builder™
 *
...
import {
  Hotel, Globe, CalendarCheck, BedDouble, ClipboardList, Sparkles as SparklesIcon,
  FileText, Wallet, Star, UserCircle2, BarChart3,
...
export const Route = createFileRoute("/_authenticated/builder/hotel")({
  head: () => ({
    meta: [
      { title: "Hotel Builder — HAPPY X" },
      { name: "robots", content: "noindex" },
    ],
  }),
  component: HotelBuilderRoute,
});

type PresetId =
  | "website" | "booking" | "rooms" | "reservations" | "housekeeping"
  | "invoices" | "payments" | "reviews" | "portal" | "analytics";

const PRESETS: { id: PresetId; label: string; icon: React.ReactNode; hint: string }[] = [
  { id: "website",       label: "Hotel Website",   icon: <Globe className="h-4 w-4" />,          hint: "Brand · rooms · amenities · gallery · SEO." },
  { id: "booking",       label: "Hotel Booking",   icon: <CalendarCheck className="h-4 w-4" />,  hint: "Search · availability · rates · book · confirm." },
  { id: "rooms",         label: "Rooms",           icon: <BedDouble className="h-4 w-4" />,      hint: "Room types · inventory · rates · amenities · media." },
  { id: "reservations",  label: "Reservations",    icon: <ClipboardList className="h-4 w-4" />,  hint: "Guests · stays · check-in · check-out · folios." },
  { id: "housekeeping",  label: "Housekeeping",    icon: <SparklesIcon className="h-4 w-4" />,   hint: "Rooms status · schedules · staff · linen · minibar." },
  { id: "invoices",      label: "Invoices",        icon: <FileText className="h-4 w-4" />,       hint: "Folios · GST · templates · dispatch · numbering." },
  { id: "payments",      label: "Payments",        icon: <Wallet className="h-4 w-4" />,         hint: "Cards · UPI · wallets · deposits · refunds · webhooks." },
  { id: "reviews",       label: "Reviews",         icon: <Star className="h-4 w-4" />,           hint: "Guest reviews · ratings · reply · moderation." },
  { id: "portal",        label: "Customer Portal", icon: <UserCircle2 className="h-4 w-4" />,    hint: "Guest login · stays · invoices · loyalty · support." },
  { id: "analytics",     label: "Analytics",       icon: <BarChart3 className="h-4 w-4" />,      hint: "Occupancy · ADR · RevPAR · funnels · cohorts." },
];

const INTRO: Record<PresetId, string> = {
  website:       "Describe the hotel site · brand · rooms · booking.",
  booking:       "Describe the booking engine · rates · rules · policy.",
  rooms:         "Describe the room type · inventory · rate · amenities.",
  reservations:  "Describe the reservation · guest · stay · folio.",
  housekeeping:  "Describe the housekeeping rule · schedule · staff.",
  invoices:      "Describe the folio · GST · template · dispatch.",
  payments:      "Describe the payment method · provider · deposit rule.",
  reviews:       "Describe the review policy · reply · moderation.",
  portal:        "Describe the guest portal · stays · invoices · loyalty.",
  analytics:     "Describe the report · metric · dimension · window.",
};

interface LogLine { id: string; at: string; kind: "log" | "warn" | "err"; text: string }

function HotelBuilderRoute() {
  ...
  const { submit: __submitPrompt } = useBuilderPrompt({ surface: "hotel", onLog: pushLog });
  ...
  return (
    <Container className="py-6 md:py-10">
      <header className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            <Hotel className="h-4 w-4" /> HAPPY Hotel Builder
          </div>
          <h1 className="text-2xl md:text-3xl font-semibold tracking-tight">
            One canonical HAPPY — rooms, reservations, housekeeping, revenue
          </h1>
      ...
      <section aria-label="Hotel presets" className="flex flex-wrap gap-2">
        {PRESETS.map((p) => (
          <Button
            key={p.id}
            size="sm"
      ...
      <div className="mt-6 grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_340px] gap-6">
        ...
        <main className="space-y-4 min-w-0">
          ...
          <Tabs defaultValue="overview" className="w-full">
            <TabsList>
              ...
              <TabsTrigger value="rooms"        className="gap-1"><BedDouble className="h-4 w-4" />Rooms</TabsTrigger>
              <TabsTrigger value="reservations" className="gap-1"><ClipboardList className="h-4 w-4" />Reservations</TabsTrigger>
              <TabsTrigger value="housekeeping" className="gap-1"><SparklesIcon className="h-4 w-4" />Housekeeping</TabsTrigger>
            ...
            <TabsContent value="overview" className="mt-3">
              <div className="rounded-lg border bg-background p-6 grid place-items-center text-sm text-muted-foreground text-center min-h-[240px]">
                <div className="flex flex-col items-center gap-2">
                  {active.icon}
                  <div>{active.label} · HAPPY renders hotel state from the Business runtime.</div>
                </div>
              </div>
            </TabsContent>
            <TabsContent value="rooms"        className="mt-3"><p className="text-sm text-muted-foreground">Room types · inventory · rates · amenities · media · availability.</p></TabsContent>
            <TabsContent value="reservations" className="mt-3"><p className="text-sm text-muted-foreground">Guests · stays · check-in · check-out · folios · upgrades.</p></TabsContent>
            <TabsContent value="housekeeping" className="mt-3"><p className="text-sm text-muted-foreground">Rooms status · schedules · staff · linen · minibar · maintenance.</p></TabsContent>
            <TabsContent value="finance"      className="mt-3"><p className="text-sm text-muted-foreground">Invoices · deposits · payments · refunds · GST · reconciliation.</p></TabsContent>
            <TabsContent value="analytics"    className="mt-3"><p className="text-sm text-muted-foreground">Occupancy · ADR · RevPAR · funnels · cohorts · reviews.</p></TabsContent>
        ...
        <aside className="space-y-3">
          <div className="flex items-center gap-2 text-sm font-medium">
            <ScrollText className="h-4 w-4" /> Hotel Log
          </div>
