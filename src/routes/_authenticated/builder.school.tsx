/**
 * /builder/school — HAPPY School Builder™
 *
...
import {
  GraduationCap, School, UserPlus, Wallet, CalendarCheck, CalendarClock,
  BookOpen, ClipboardCheck, Users, Presentation, User, Award,
...
export const Route = createFileRoute("/_authenticated/builder/school")({
  head: () => ({
    meta: [
      { title: "School Builder — HAPPY X" },
      { name: "robots", content: "noindex" },
    ],
  }),
  component: SchoolBuilderRoute,
});

type PresetId =
  | "erp" | "admission" | "fees" | "attendance" | "timetable" | "homework"
  | "results" | "parents" | "teacher" | "student" | "certificates";

const PRESETS: { id: PresetId; label: string; icon: React.ReactNode; hint: string }[] = [
  { id: "erp",          label: "School ERP",       icon: <School className="h-4 w-4" />,          hint: "Classes · sections · staff · calendar · policies." },
  { id: "admission",    label: "Admission",        icon: <UserPlus className="h-4 w-4" />,        hint: "Enquiry · forms · tests · approval · enrollment." },
  { id: "fees",         label: "Fees",             icon: <Wallet className="h-4 w-4" />,          hint: "Heads · schedules · invoices · payments · dues." },
  { id: "attendance",   label: "Attendance",       icon: <CalendarCheck className="h-4 w-4" />,   hint: "Daily · period · biometric · parent alerts." },
  { id: "timetable",    label: "Timetable",        icon: <CalendarClock className="h-4 w-4" />,   hint: "Classes · periods · rooms · teacher load." },
  { id: "homework",     label: "Homework",         icon: <BookOpen className="h-4 w-4" />,        hint: "Assign · submit · grade · feedback · reminders." },
  { id: "results",      label: "Results",          icon: <ClipboardCheck className="h-4 w-4" />,  hint: "Exams · marks · grades · report cards · publish." },
  { id: "parents",      label: "Parents Portal",   icon: <Users className="h-4 w-4" />,           hint: "Progress · fees · attendance · messages · meetings." },
  { id: "teacher",      label: "Teacher Portal",   icon: <Presentation className="h-4 w-4" />,    hint: "Classes · attendance · homework · marks · comms." },
  { id: "student",      label: "Student Portal",   icon: <User className="h-4 w-4" />,            hint: "Timetable · homework · results · library · fees." },
  { id: "certificates", label: "Certificates",     icon: <Award className="h-4 w-4" />,           hint: "Bonafide · TC · character · templates · issue." },
];

const INTRO: Record<PresetId, string> = {
  erp:          "Describe the school · classes · sections · policies.",
  admission:    "Describe the admission flow · form · test · approval.",
  fees:         "Describe the fee head · schedule · rule · dues.",
  attendance:   "Describe the attendance rule · mode · alerts.",
  timetable:    "Describe the timetable · periods · rooms · load.",
  homework:     "Describe the homework · subject · rubric · deadline.",
  results:      "Describe the exam · grading · report card · publish.",
  parents:      "Describe the parent portal · view · alerts.",
  teacher:      "Describe the teacher portal · classes · tools.",
  student:      "Describe the student portal · view · tools.",
  certificates: "Describe the certificate · template · issue rule.",
};

interface LogLine { id: string; at: string; kind: "log" | "warn" | "err"; text: string }

function SchoolBuilderRoute() {
  ...
  const { submit: __submitPrompt } = useBuilderPrompt({ surface: "school", onLog: pushLog });
  ...
  return (
    <Container className="py-6 md:py-10">
      <header className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            <GraduationCap className="h-4 w-4" /> HAPPY School Builder
          </div>
          <h1 className="text-2xl md:text-3xl font-semibold tracking-tight">
            One canonical HAPPY — admissions, academics, fees, portals
          </h1>
      ...
      <section aria-label="School presets" className="flex flex-wrap gap-2">
        {PRESETS.map((p) => (
          <Button
            key={p.id}
            size="sm"
      ...
      <div className="mt-6 grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_340px] gap-6">
        ...
        <main className="space-y-4 min-w-0">
          ...
          <Tabs defaultValue="overview" className="w-full">
            <TabsList>
              ...
              <TabsTrigger value="academics" className="gap-1"><BookOpen className="h-4 w-4" />Academics</TabsTrigger>
              ...
              <TabsTrigger value="portals"   className="gap-1"><User className="h-4 w-4" />Portals</TabsTrigger>
            ...
            <TabsContent value="overview" className="mt-3">
              <div className="rounded-lg border bg-background p-6 grid place-items-center text-sm text-muted-foreground text-center min-h-[240px]">
                <div className="flex flex-col items-center gap-2">
                  {active.icon}
                  <div>{active.label} · HAPPY renders school state from the Business runtime.</div>
                </div>
              </div>
            </TabsContent>
            <TabsContent value="academics" className="mt-3"><p className="text-sm text-muted-foreground">Classes · timetable · homework · exams · results · certificates.</p></TabsContent>
            <TabsContent value="people"    className="mt-3"><p className="text-sm text-muted-foreground">Students · teachers · parents · staff · admission · attendance.</p></TabsContent>
            <TabsContent value="finance"   className="mt-3"><p className="text-sm text-muted-foreground">Fee heads · schedules · invoices · payments · dues · refunds.</p></TabsContent>
            <TabsContent value="portals"   className="mt-3"><p className="text-sm text-muted-foreground">Parents · teachers · students · messages · meetings.</p></TabsContent>
            <TabsContent value="analytics" className="mt-3"><p className="text-sm text-muted-foreground">Attendance · performance · fees collection · cohorts.</p></TabsContent>
        ...
        <aside className="space-y-3">
          <div className="flex items-center gap-2 text-sm font-medium">
            <ScrollText className="h-4 w-4" /> School Log
          </div>
