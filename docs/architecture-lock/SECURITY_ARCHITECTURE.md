...
## Summary
Zero-trust: RLS everywhere, security-definer helpers, cron-shared-secret (`CRON_SHARED_SECRET`), TTS auth + rate limit, PGREST filter sanitization (`src/lib/security/`). AppError leaks removed.
